...
# #750: --reconcile is branch-scoped by default; these single-commit fixtures
# have no feature branch (HEAD == integration base), so --all is required to
# exercise the configured-enum flip mechanics this test pins (scope itself is
# pinned by _smoke-test-check7-branch-scope.sh / HC-10).
set +e
HEALTH_OUT=$(bash "$CHECK" --reconcile --all "$FIXTURE" 2>&1)
HEALTH_RC=$?
set -e
...
EOF
set +e
SUB_OUT=$(bash "$CHECK" --reconcile --all "$FIXTURE2" 2>&1)
set -e
