...
# GitHub issue. The HTML-comment marker lets refresh-next-cache.sh and
...
#
# Exit: 0 posted; 1 bad args / gh missing; 2 gh call failed.
set -euo pipefail
...
command -v gh >/dev/null 2>&1 || { echo "post-handoff-comment.sh requires the gh CLI" >&2; exit 1; }
...
( cd "$PROJECT_DIR" && gh issue comment "$ISSUE" --body-file "$body" ) \
  || { echo "gh issue comment failed for #$ISSUE" >&2; exit 2; }
