...
```
<model>  |  ts HH:MM  |  <project>/<branch>  |  ctx <N>%  |  5h:<N>% 7d:<N>%  |  wt:<name>  |  [#<issue> <stage>]
```
...
- The consumer MUST re-scrub the `stage` field (mirror of the producer's `_CTRL` regex) before render, as belt-and-braces against a hand-edited or older-version cache. (`statusline.sh` does this — `scrub()` ~line 61, applied to `stage` at chip assembly and to model / project / branch / worktree.)
...
- `.arboretum/active-stage-cache.json` — refresh-stage-cache seam: `{issue, stage, ts}`. Background-refreshed past a 30 s TTL.
- No CLI args.
...
- **Chip format (stage-cache consumer).** `[#<issue> <stage>]` only when both `issue` truthy AND `stage` non-empty; `[#<issue>]` when only `issue`; no chip otherwise (absent cache, unparseable cache, or both fields null/empty).
