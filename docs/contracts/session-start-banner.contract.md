...
- **Scrub invariant — scrubbed blocks.** Author-controlled strings reaching `additionalContext` are control-char scrubbed (`\x00-\x08\x0b\x0c\x0e-\x1f\x7f-\x9f`) at the consumer (this hook) as defense-in-depth, mirroring the producer-side scrub in each cache writer. The hook applies a `scrub()` helper:
  - **Next-up block** — `def scrub` (~line 123) applied to the handoff next-action/body and the error diagnostic. (#763: the block renders only the bare `#N`; issue title/body/url are no longer rendered into `additionalContext`, so they are no longer scrub targets here. The handoff note is the only author-controlled free text the block now feeds the model, and it is author-gated at the producer — `refresh-next-cache.contract.md` RNC-8.)
...
- **SSB-2:** Scrub invariant (scrubbed block) — a next-cache **handoff `next_action`** carrying a raw ESC (`0x1b`) control byte renders into the `[Next-up]` block's note with the ESC byte stripped (consumer re-scrub), printable residue preserved; the bare `[Next-up] #N` line also renders. (#763: the assertion moved off the issue title, which no longer renders, onto the handoff note — the only author-controlled free text the block still feeds the model.)
...
- **1.6** (2026-06-12) — neutralised the `[Next-up]` block's prompt-injection surface (#763): the block renders only the bare `#N`; issue title/body/url are no longer rendered into `additionalContext`. The (author-gated, RNC-8) handoff note is retained as the block's only author-controlled free text. Next-up scrub clause narrowed to the handoff fields + error diagnostic; SSB-2 re-pointed from the issue title to the handoff `next_action`.
