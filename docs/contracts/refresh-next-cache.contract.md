...
The seam between `scripts/refresh-next-cache.sh` (the producer of `.arboretum/next-cache.json` — the cached snapshot of the GitHub issue tagged `next-up`, refreshed on session start with a 1-hour TTL) and its sole downstream consumer `.claude/hooks/session-start.sh` (the `[Next-up]` block of the SessionStart hook's boot banner). One folded-in bug closes as non-recurrable: **#264** — the second of two `gh` calls (`gh issue view <N> --json comments`) silently fell back to an empty comments file on failure, producing `handoff: null` indistinguishable from "no handoff comment exists." RNC-4 makes that conjunction a write-time impossibility — the contract test pins that any non-zero exit from the comment fetch produces the explicit error union variant.

## Producer

`scripts/refresh-next-cache.sh` — producer-type: `script`.

Refreshes the cache at `.arboretum/next-cache.json` by making up to two `gh` calls: one `gh issue list --label next-up --state open --limit 1 --json number,title,url,body,labels,updatedAt` (the primary list call) and, when an issue is found, one follow-up `gh issue view <N> --json comments` to extract the latest `arbo-handoff`-marked comment as the current session-handoff note (per `session-handoff` design §4.6).
...
The two `gh` calls degrade in coordinated ways: the primary list call's failure sets the whole-cache `error` field (`gh-unavailable` or `gh-call-failed`), invalidating the cache as a whole; the secondary comment-fetch call's failure sets a *scoped* error marker on the `handoff` field, preserving the valid issue data above it.
...
- **`[project-dir]`** — positional, defaults to `git rev-parse --show-toplevel` or `pwd`. Sets the root under which `.arboretum/next-cache.json` is written and the project's `gh` repo context (via `cd $PROJECT_DIR` inside subshells) is established.

Reads (under the project-dir root):

- `git remote` — to detect whether `gh` should be invoked at all (`no_gh_remote: true` short-circuit when no remote is configured).
- `gh` CLI — must be installed (`command -v gh`) and authenticated (`gh auth status` exits 0). Absence or auth failure sets whole-cache `error: "gh-unavailable"` and exits 1.
- `gh issue list --label next-up --state open --limit 1 --json number,title,url,body,labels,updatedAt` — the primary list call. Failure (other than "not a GH repo") sets whole-cache `error: "gh-call-failed"` and exits 2.
- `gh issue view <N> --json comments` — the secondary comment-fetch call, made only when the primary list returned an issue. Failure sets `handoff: {"error": "fetch-failed", "detail": <stderr-first-line>}` and the script continues to exit 0.
...
```json
{
  ...
  "handoff": null
            ...
            | { "error": "fetch-failed",
                "detail": "<first line of gh stderr, control-char-stripped>" },
...
- `0` — cache written successfully. Sub-cases: issue found and handoff fetched OK; issue found and no handoff comment exists; issue found and handoff fetch failed (error union recorded in cache); no issue carries the `next-up` label; no GH remote configured.
- `1` — `gh` CLI missing OR `gh` unauthenticated. Whole-cache `error: "gh-unavailable"` recorded.
- `2` — primary `gh issue list` call failed for some other reason. Whole-cache `error: "gh-call-failed"` recorded.
...
- **Exit-code contract.** Comment-fetch failure is exit `0` (cache successfully written; failure is recorded *in* the cache, not *about* the cache). Whole-cache `gh` failures are exit `1`/`2` per the table above.
...
- **Comment-fetch failure discipline.** When `gh issue view <N> --json comments` exits non-zero, the cache records `handoff: {"error": "fetch-failed", "detail": ...}` — NEVER `handoff: null` (which would be indistinguishable from "no handoff comment exists").
...
- **RNC-2: Exit-code contract.** `bash scripts/refresh-next-cache.sh [dir]` exits 0 when the cache was successfully written (any of: issue found and handoff fetched OK; issue found and no handoff comment exists; issue found and comment fetch failed with the error-union recorded; no issue carries `next-up`; no GH remote), 1 when `gh` is missing/unauthenticated, 2 when the primary `gh issue list` call fails. Comment-fetch failure is exit 0. The smoke test asserts the exit-code transition between the success-and-no-handoff case (0), the comment-fetch-failure case (0 — same), and any whole-cache failure (1 or 2) via gh-stub manipulation.
...
- **RNC-6: ANSI-scrub-invariant.** All author-controlled string fields in the cache are control-char-stripped. The smoke test injects a synthetic ANSI escape (e.g. `\x1b[31m`) into the gh-stub's comment-fetch stderr and asserts the cache's `handoff.detail` does not contain the raw escape sequence.
