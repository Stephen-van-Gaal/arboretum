...
1. **strategic-review-due** (weekly; gh-independent) — fires when `now - last_reviewed >= review_cadence_weeks * 7` days.
2. **maintain-overdue** (daily; needs gh) — fires when `last_maintain_run` > 7d ago AND `>= 3` untriaged issues.
3. **stale-flagged-today** (daily; needs gh) — fires when `>= 1` `provisionally-stale` open issue exists.
4. **agent-ready-while-WIP-full** (daily; needs gh) — fires when WIP `>= wip_limit` AND `>= 1` agent-ready issue.
5. **profile-graduation-lean** (weekly; needs gh) — fires when `profile=minimal` AND `>= 20` open issues.

Each fired nag prints one `[nag] <message>` line and records its name; after all evaluation, fire timestamps are batch-written to the pulse (so a fired nag is throttled on the next run). The gh-dependent nags (2–5) are silently skipped when `gh` is missing or unauthenticated.

## Consumer

Consumer-type: `script`. Two downstream consumers, both capturing stdout via command substitution:

- **`scripts/roadmap/build-orientation.sh`** (~line 18) `NAG_OUTPUT="$(bash "$SCRIPT_DIR/nag.sh" 2>/dev/null || true)"`, printed after the orientation block (and on the no-gh early-exit path so review nags surface offline).
- **`scripts/roadmap/render-run.sh`** (~line 47) `nag_output="$(bash "$SCRIPT_DIR/nag.sh" 2>/dev/null || true)"`, appended to both the full view (trailing) and the no-gh early-exit path.
...
- gh-dependent nags additionally query `gh issue list` (untriaged / provisionally-stale / horizon:now / agent-ready / open counts).
...
- **Always exit 0.** The script exits 0 in every path, including missing lib, missing config, missing pulse, missing/unauthenticated gh.
- **Config-gated.** With no resolvable `roadmap.config.yaml`, the script exits 0 with empty stdout (no nags).
- **gh-independence of nag 1.** strategic-review-due is computed without gh; nags 2–5 are silently skipped when gh is unavailable.
...
- **RN-1:** In a roadmap fixture with `last_reviewed` well past `review_cadence_weeks` and an old pre-seeded pulse (so the install-day quiet guarantee doesn't suppress the nag), nag.sh emits exactly one `[nag] …` line (strategic-review-due) with the `[nag] ` prefix; exit 0. (gh-independent — gh is omitted from PATH.)
