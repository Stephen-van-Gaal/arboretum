...
| fresh-context-driver | A unit of work run off the main thread by dispatching a generic subagent and briefing it to invoke a skill or inline a procedure, returning only its structured result. | skill-and-agent-authoring | `docs/specs/skill-and-agent-authoring.spec.md` Fresh-context driver dispatch section; public fallback `skills/finish/SKILL.md` Step 5, `skills/cleanup/SKILL.md` Step 3, and `skills/ai-surface-review/SKILL.md` Dispatch section | generic-subagent, driver, conductor (#629) | The skill/lane/step name is never a `subagent_type`; /finish, /cleanup, and the ai-surface SKILL dispatch prose must stay synchronized. |
...
| autonomy-grant | Per-run authorization, set by a human at the design→build grant gate, naming how far a run proceeds unattended; surfaced as a closed `autonomy:*` tier label and carried on the pipeline-state seam. | autonomy-grants | `docs/specs/autonomy-grants.spec.md`; public fallback `scripts/roadmap/install-labels.sh`, `scripts/read-autonomy-config.sh`, and `scripts/read-autonomy-grant.sh` | autonomy:pause-at-land, autonomy:pause-at-merge, autonomy:auto-merge, design-only, grant gate, trigger floor | The grant vocabulary, the `.arboretum.yml autonomy:` parameters, and the design-gate prose must stay synchronized; AI advises but never grants (D1). |
...
| promotion-contract | The design→spec seam pinning what design-spec intent must be promoted into a permanent spec's HUMAN sections at creation; provenance only, never substitution. | pipeline-contracts-template | `docs/contracts/promotion.contract.md` | promotion, design-to-spec promotion, provenance-only | The spec template (what a rich spec carries) and `/design` mechanics (#686, when/how) must preserve the what-crosses vs when/how split. |
| spec-area | An intra-spec facet (retrieval-only) declared in the `areas:` frontmatter block; declaring ≥1 area forces an explicit area-vs-split assessment. | document-taxonomy | `docs/templates/spec.md` `areas:` frontmatter | area, facet, split-assessment, area-vs-split gate | Spec template, the group layer (#681), and split-gate validators (#685) must agree on area-vs-subsystem semantics. |
| quality-attributes | A governed spec's optional component NFRs expressed as testable scenarios; declare N/A when absent. | document-taxonomy | `docs/templates/spec.md` `Quality Attributes` section | NFR, non-functional requirement, quality requirement | Project-wide NFRs stay in ARCHITECTURE; the spec section carries only component NFRs. |
| seam-direction | A spec's `Requires`/`Provides` are the inbound/outbound seam; a contract's `consumes`/`produces` are the same concept at the shared-contract altitude (`Requires`≈`consumes`, `Provides`≈`produces`). Documented mapping, not a rename. | pipeline-contracts-template | `docs/templates/spec.md` Requires/Provides comments | Requires, Provides, consumes, produces, inbound seam, outbound seam | Spec templates and contract frontmatter keep the mapping documented; do not introduce a third vocabulary. |
...
|---|---|---|---|
| 2026-06-08 | v2 | Additive: added promotion-contract, spec-area, quality-attributes, and seam-direction anchors for the component-spec content model (#671). No schema change — existing `@v2` pins remain valid. | document-taxonomy, pipeline-contracts-template |
