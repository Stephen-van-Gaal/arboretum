...
| AI-facing session document | `docs/templates/design-spec.md` | `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md` | Provider-owned build support | Detailed design/session artifact for implementation context and S2 handoff. Arboretum requires only the handoff fields needed by `/build`; body shape may vary by provider. | Provider invoked by `/design`; before `design-package` and `/build`. |
| Human session overview | generated by `design-package` | Displayed during `/design`; may be embedded or retained when useful | Arboretum-owned review packet | Compact human control surface: why the session exists, what changes, durable document impact, review points, tests/confidence, and stop conditions. | Produced by `design-package` after the AI-facing session document exists. |
| Durable Document Change Set | generated by `design-package` from the session document and survey | Section inside the human overview | Arboretum-owned review packet | Exact durable files, operation, high-level change, why it matters, and phase classification. Separates intent/seam authority from generated/evidence authority. | Reviewed with the durable-doc diff before `/build`. |
...
- If it explains what the human is being asked to approve for the current
  session, put it in the human session overview.
- If it lists durable files that will be created, modified, or deferred, put it
  in the Durable Document Change Set.
...
- `/design` delegates detailed build-specific artifacts to the configured
  provider: currently an AI-facing session document and plan from Superpowers.
  Arboretum validates the S2 handoff fields, then invokes `design-package` to
  produce the human overview and Durable Document Change Set. Approved
  intent/seam durable edits may be committed before `/build`; generated/evidence
  authority waits for built evidence.
...
In short: providers author detailed session context, `design-package` creates
the human review packet, build produces evidence, and consolidate makes
generated/evidence Arboretum docs current.
